...
const PLAYERS = 20;
const ROOMS = 10;
const TEST_PREFIX = "loadtest.domino2.full";
...
async function run() {
  ...
  const report = () => {
    ...
    console.log("=== LOADTEST FULL RESULT ===");
  ...
  try {
    console.log(`[full-loadtest] auth bootstrap for ${PLAYERS} users...`);
    ...
    console.log(`[full-loadtest] creating ${ROOMS} stake rooms and joining 2 players each...`);
    ...
    console.log("[full-loadtest] reconnect probe in all 10 rooms...");
    for (const pair of roomPairs) {
      const token = String(pair.host?.reconnectionToken || "").trim();
      if (!token) {
      ...
      try {
        pair.host.connection?.close(4001, "full_loadtest_disconnect");
        await sleep(120);
        let resumed = null;
        try {
          resumed = await client.reconnect(token);
        } catch {
          resumed = await client.joinById(pair.roomId, {
        ...
        attachAutoPilot(resumed, pair.stats, metrics.errors);
        pair.host = resumed;
        pair.stats.reconnectOk = true;
        openRooms.push(resumed);
        metrics.reconnectOk += 1;
      } catch (error) {
        metrics.errors.push(`reconnect room=${pair.roomId}: ${error.message}`);
      }
      await sleep(80);
    }

    console.log("[full-loadtest] playing for up to 5 minutes...");
...
run().catch((error) => {
  console.error("[full-loadtest] fatal:", error);
  process.exitCode = 1;
});
