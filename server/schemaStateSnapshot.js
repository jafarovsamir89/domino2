function buildSchemaStateSnapshotData({ state, roomMode = "", scoreMode = "" }) {
    ...
    return {
        ...
        deal: state?.deal,
        gameMode: String(state?.gameMode || state?.mode || "telefon").trim() || "telefon",
        mode: String(state?.mode || "telefon").trim() || "telefon",
        matchStateJson: String(state?.matchStateJson || ""),
...
function buildRestoredSchemaStateData({ snapshot, currentState, totalPlayers, sanitizeName }) {
    ...
    return {
        ...
        gameMode: String(snapshot?.gameMode || snapshot?.mode || "telefon").trim() || "telefon",
        mode: String(snapshot?.mode || "telefon").trim() || "telefon",
        matchStateJson: String(snapshot?.matchStateJson || ""),
