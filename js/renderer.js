export class Renderer {
    ...
    createTileEl(a, b, orient='horizontal', small=false, id=null) {
        ...
        let halfIdx = 0;
        for (const v of [a, b]) {
            const half = document.createElement('div'); half.className = 'tile-half';
            const pos = this.pipLayout(v, orient);
            for (let i = 0; i < 9; i++) { const p = document.createElement('div'); p.className = pos.includes(i)?'pip':'pip hidden'; half.appendChild(p); }
            el.appendChild(half);
            if (halfIdx === 0) { const d = document.createElement('div'); d.className = 'tile-divider'; el.appendChild(d); }
            halfIdx++;
        }
        return el;
    }
    ...
    renderOpponentHands(hands, hi, names, cur=-1) {
        ...
        for (let i = 0; i < hands.length; i++) {
            ...
            g.appendChild(l);
            for (let j = 0; j < hands[i].length; j++) { const t = document.createElement('div'); t.className = 'opp-tile'; g.appendChild(t); }
    ...
    renderBoneyard(count) {
        ...
        if (!count) return;
        const stack = document.createElement('div'); stack.className = 'boneyard-stack';
        for (let i = 0; i < Math.min(count, 4); i++) {
            const t = document.createElement('div'); t.className = 'stack-tile';
            t.style.cssText = `top:${i*2}px;left:${i}px;`; stack.appendChild(t);
        }
        const lbl = document.createElement('div'); lbl.className = 'boneyard-count'; lbl.textContent = count;
        this.boneyardVisual.appendChild(stack); this.boneyardVisual.appendChild(lbl);
    }
    ...
    renderBoard(board) {
        ...
        if (!board.nodes.length) {
            ...
            ph.textContent = 'Сделайте первый ход';
            this.boardEl.appendChild(ph); return;
        }
        ...
        const pad=20, lw=(mxX-mnX)+pad*2, lh=(mxY-mnY)+pad*2;
        const bcRect = bc.getBoundingClientRect();
        // Reserve space for side opponent containers
        ...
        // Zoom: scale to fit instead of scroll
        ...
        const ox = (mxX+mnX)/2, oy = (mxY+mnY)/2;
        this._lastOx = ox;
        this._lastOy = oy;
        ...
        for (let i = 0; i < board.nodes.length; i++) {
            ...
            el.classList.add('board-tile'); // Use CSS class for centering
    ...
    // Show semi-transparent arrows on board for direction choice
    showArrowChoices(board, matchingEnds, onChoose, onCancel) {
        ...
        overlay.addEventListener(tapEvent, (e) => { if (e.target === overlay) { this.removeArrows(); onCancel(); } });

        for (const ei of matchingEnds) {
            const oe = board.openEnds[ei];
            const node = board.nodes[oe.nodeId];
            // Position arrow near the open end
            const btn = document.createElement('button');
            btn.className = 'arrow-btn';
            btn.textContent = `${arrowSymbols[oe.side]||'?'}`;
            btn.title = `Поставить к ${oe.value}`;
            ...
            btn.addEventListener(tapEvent, (e) => { e.stopPropagation(); this.removeArrows(); onChoose(ei); });
    ...
    renderHand(hand, validMoves=[], sel=-1, isCurrent=false) {
        ...
        for (let i = 0; i < hand.length; i++) {
            ...
            if (validMoves.some(m => m.tileIndex === i)) { el.classList.add('playable'); el.dataset.handIndex = i; }
    ...
    renderScores(players, cur) {
        ...
        for (const p of players) {
            const it = document.createElement('div'); it.className = 'score-item';
            ...
            it.innerHTML = `${teamTag}<span class="score-name">${p.name}:</span> <span class="score-value">${p.score}</span><span class="score-wins"> ${p.roundWins}р</span>`;
            this.scoresEl.appendChild(it);
        }
    }

    renderInfo(mr, deal, by, sum) {
        const rText = this.app.currentLang === 'az' ? 'R' : (this.app.currentLang === 'ru' ? 'Р' : 'R');
        const sText = this.app.currentLang === 'az' ? 'S' : (this.app.currentLang === 'ru' ? 'С' : 'D');
        this.roundInfoEl.textContent = `${rText}${mr}/3 · ${sText}${deal}`;
        this.boneyardInfoEl.textContent = `Σ${sum}`;
        this.renderBoneyard(by);
    }

    showGoshaBtn(combo, onGoshaClick) {
        let btn = document.getElementById('gosha-btn');
        if (!combo) { if (btn) btn.remove(); return; }
        if (!btn) { btn = document.createElement('button'); btn.id = 'gosha-btn'; btn.className = 'btn btn-gosha'; document.querySelector('.action-bar').appendChild(btn); }
        btn.textContent = `🎲 Гоша ×${combo.matches.length} (+${combo.score})`;
        btn.onclick = () => { if (onGoshaClick) onGoshaClick(); };
    }

    showMessage(t, dur=0) { this.messageEl.textContent = t; if (dur>0) setTimeout(() => { if (this.messageEl.textContent===t) this.messageEl.textContent=''; }, dur); }
    clearMessage() { this.messageEl.textContent = ''; }
    showScorePopup(pts) {
        const p = document.createElement('div'); p.className = 'score-pop'; p.textContent = `+${pts}`;
        p.style.cssText = 'left:50%;top:35%;'; document.getElementById('game-screen').appendChild(p);
        setTimeout(() => p.remove(), 1500);
    }
    setDrawEnabled(e) { this.drawBtn.disabled = !e; }
    setPassEnabled(e) { this.passBtn.disabled = !e; }

    renderDealEnd(wn, players, fish, bonus) {
        document.getElementById('round-end-title').textContent = fish ? this.app.t('msg-fish') : `${wn} ${this.app.currentLang === 'az' ? 'çıxdı!' : (this.app.currentLang === 'ru' ? 'вышел!' : 'out!')}`;
        const d = document.getElementById('round-end-details'); d.innerHTML = '';
        for (const p of players) {
            const r = document.createElement('div'); r.className = 'detail-row';
            r.style.flexDirection = 'column'; r.style.alignItems = 'flex-start'; r.style.gap = '6px';
            
            const top = document.createElement('div');
            top.style.display = 'flex'; top.style.justifyContent = 'space-between'; top.style.width = '100%';
            let v = `Рука: ${p.handPoints}`;
            if (p.isWinner) v += ` · Бонус: +${bonus}`;
            v += ` · Итого: ${p.score}`;
            ...
            if (p.leftoverHands) {
                ...
                handsDiv.style.display = 'flex'; handsDiv.style.flexWrap = 'wrap'; handsDiv.style.gap = '4px';
        ...
        document.getElementById('next-round-btn').textContent = 'Следующая сдача';
        document.getElementById('round-end-screen').classList.add('active');
    }
    renderRoundEnd(wn, players, wins, mr, over) {
        document.getElementById('round-end-title').textContent = wins>=2 ? `${wn} ×2! 🔥` : `${wn} берёт раунд! 🏅`;
        const d = document.getElementById('round-end-details'); d.innerHTML = '';
        for (const p of players) {
            const r = document.createElement('div'); r.className = 'detail-row';
            let v = `Очки: ${p.score} · Раунды: ${p.roundWins}`; if (p.isWinner) v += ` (+${wins})`;
            r.innerHTML = `<span>${p.name}</span><span class="detail-value">${v}</span>`; d.appendChild(r);
        }
        document.getElementById('next-round-btn').textContent = over ? 'Итоги' : 'Следующий раунд';
        document.getElementById('round-end-screen').classList.add('active');
    }
    showInstantWin(pn, s) {
        document.getElementById('round-end-title').textContent = `💥 ${pn}: ${s} за ход! +2 раунда!`;
        document.getElementById('round-end-details').innerHTML = '<div class="detail-row"><span>Мгновенная победа!</span></div>';
        document.getElementById('next-round-btn').textContent = 'Итоги';
        document.getElementById('round-end-screen').classList.add('active');
    }
    renderGameOver(wn, players) {
        document.getElementById('game-over-title').textContent = `${wn} ${this.app.currentLang === 'az' ? 'qalib gəldi!' : (this.app.currentLang === 'ru' ? 'победил!' : 'won!')} 🏆`;
        const d = document.getElementById('game-over-details'); d.innerHTML = '';
        for (const p of [...players].sort((a,b)=>b.roundWins-a.roundWins)) {
            const r = document.createElement('div'); r.className = 'detail-row';
            r.innerHTML = `<span>${p.name}</span><span class="detail-value">${p.roundWins} р</span>`; d.appendChild(r);
        }
